...
export default function FSRScreen() {
  ...
  const [cableCondition, setCableCondition] = useState<"Good" | "Bad" | null>(
    null,
  );
  ...
  const [potentialShading, setPotentialShading] = useState<"Yes" | "No" | null>(
    null,
  );
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      submitted_at: performedAt,
      cable_condition: cableCondition ?? "",
      cable_quantity: cableQuantity.trim(),
      panel_damage: panelDamage ?? "",
      panel_brand: panelBrand.trim(),
      inverter_alarm: inverterAlarm ?? "",
      alarm_code: alarmCode.trim(),
      potential_shading: potentialShading ?? "",
      shading_details: shadingDetails.trim(),
      rusting: String(rusting),
      bird_dropping: String(birdDrop),
      mos_and_debris: String(mosDebris),
      earthing: String(earthing),
    };
    ...
    >
      <View style={styles.root}>
        ...
        <View style={styles.header}>
          <TouchableOpacity
            onPress={() =>
              Alert.alert("Go Back?", "Unsaved progress will be lost.", [
        ...
        >
          ...
          <Section label="PANELS CLEANED" required>
            <View style={styles.stepper}>
              ...
              <TextInput
                ...
                value={panelsCleaned}
                onChangeText={(v) => setPanelsCleaned(v.replace(/[^0-9]/g, ""))}
          ...
          <Section label="CABLE CONDITION" required>
            <Toggle
              options={["Good", "Bad"] as const}
              value={cableCondition}
              onChange={setCableCondition}
              activeColors={[ACCENT2, DANGER]}
            />
          </Section>
          ...
          <Section label="PANEL DAMAGE" required>
            <Toggle
              ...
              value={panelDamage}
              onChange={setPanelDamage}
              activeColors={[DANGER, ACCENT2]}
            />
          </Section>
          ...
          <Section label="POTENTIAL SHADING" required>
            <Toggle
              options={["Yes", "No"] as const}
              value={potentialShading}
              onChange={setPotentialShading}
              activeColors={[WARNING, ACCENT2]}
            />
          </Section>
          ...
          {potentialShading === "Yes" && (
            <Section label="SHADING DETAILS">
              <TextInput
                ...
                value={shadingDetails}
                onChangeText={setShadingDetails}
                placeholder="Describe the shading source…"
                placeholderTextColor={TEXT_DIM}
                multiline
                numberOfLines={3}
          ...
          <Section label="SITE CONDITIONS">
            <CheckRow
              label="Rusting observed"
              value={rusting}
              onChange={setRusting}
            />
            <CheckRow
              label="Bird dropping present"
              value={birdDrop}
              onChange={setBirdDrop}
            />
            <CheckRow
              label="Moss & debris present"
              value={mosDebris}
              onChange={setMosDebris}
            />
            <CheckRow
              label="Earthing in place"
              value={earthing}
