...
export default function SLDMapScreen({ navigation, route }: any) {
  ...
  // ─── Fire event ───────────────────────────────────────────────────────────────
  // NOTE: site_exited is NOT handled here — FSRScreen owns that PATCH + navigation.
  ...
  // ── site_exited goes to FSRScreen; all other events go through fireEvent ──────
  ...
  const getPinStyle = (point: Point) => {
    ...
    return { left: oX + (point.x_percent / 100) * rW - PIN_SIZE / 2, top: oY + (point.y_percent / 100) * rH - PIN_SIZE / 2 };
  };
  ...
  const allPinsDone = points.length > 0 && doneCount === points.length;
  const pinsLocked = jobStep < 1;
      ...
      <View style={styles.ctaCard}>
        <View style={styles.ctaCardLeft}>
          ...
          <View>
            ...
            <Text style={styles.ctaSubtitle}>Tap to unlock the diagram</Text>
          </View>
        </View>
    ...
    <LinearGradient colors={["#080C18", "#0D1120"]} style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
        ...
        >
          ...
          {!imageLoading && points.map((point) => {
            const done = pinDone(point.id);
            return (
              <TouchableOpacity
                key={point.id}
                style={[styles.pin, getPinStyle(point), done && styles.pinDone, pinsLocked && styles.pinLocked]}
                onPress={() => openPin(point)}
                activeOpacity={pinsLocked ? 1 : 0.8}
              >
                <Ionicons name={done ? "checkmark" : "ellipse"} size={16} color={pinsLocked ? "#2D3A50" : done ? "#080C18" : "#fff"} />
              </TouchableOpacity>
            );
          })}
          ...
          {pinsLocked && !imageLoading && (
            <View style={styles.diagramLockOverlay}>
              <View style={styles.diagramLockCard}>
                ...
                <Text style={styles.diagramLockText}>Tap REACHED PANELS{"\n"}to unlock the diagram</Text>
              </View>
            </View>
          )}
        </View>
        ...
        {jobStep >= 1 && (
          <View style={styles.legend}>
            ...
            <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: "#22D3A5" }]} /><Text style={styles.legendText}>Done</Text></View>
            <Text style={styles.legendHint}>Tap pin to start</Text>
          </View>
        )}
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHeader}>
              <View>
                <Text style={styles.modalTitle}>{selectedPin?.label ?? `Pin #${selectedPin?.id}`}</Text>
...
const styles = StyleSheet.create({
  ...
  pin: { position: "absolute", width: PIN_SIZE, height: PIN_SIZE, borderRadius: PIN_SIZE / 2, backgroundColor: "#0EA5E9", justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: "#fff", shadowColor: "#0EA5E9", shadowOpacity: 0.8, shadowRadius: 8, elevation: 6 },
  pinDone: { backgroundColor: "#22D3A5", borderColor: "#22D3A5", shadowColor: "#22D3A5" },
  pinLocked: { backgroundColor: "#1E2A45", borderColor: "#2D3A50", shadowOpacity: 0, elevation: 0 },
  legend: { flexDirection: "row", alignItems: "center", gap: 16, paddingHorizontal: 16, paddingVertical: 10, borderTopWidth: 1, borderTopColor: "#1E2A45", backgroundColor: "#0D1120" },
  legendItem: { flexDirection: "row", alignItems: "center", gap: 6 },
  legendDot: { width: 10, height: 10, borderRadius: 5 },
  legendText: { fontSize: 12, color: "#94A3B8" },
  legendHint: { marginLeft: "auto" as any, fontSize: 11, color: "#475569", fontStyle: "italic" },
